...
import { COMPARES } from "@/app/compare/[slug]/compareData";
...
Nimbus uses per-warehouse pricing across three plans. All plans include
...
This is the canonical reference for what Nimbus does. Cover the headline
capabilities so Claude can answer "what is Nimbus" / "what does it do"
...
function formatCompare([slug, data]) {
  const reasons = (data.reasons || [])
    ...
    .join("\n");
  const competitor = data.competitorName || slug;
  return [
    `## Nimbus vs ${competitor}`,
    `Source: ${SITE_URL}/compare/${slug}`,
    "",
    data.heroDesc || "",
    reasons
      ? `\nWhy teams choose Nimbus over ${competitor}:\n${reasons}`
      : null,
    "",
  ]
    .filter((x) => x !== null)
...
function buildKnowledgeBase() {
  ...
  out.push("# NIMBUS WMS KNOWLEDGE BASE");
  out.push("");
  out.push(
    "This is the complete Nimbus content library. Each section includes a Source URL — reference it naturally when answering ('see /pricing for the breakdown'). Never invent details that aren't in here; if something isn't covered, say so plainly."
  );
  out.push("");
  ...
  const compares = Object.entries(COMPARES ?? {});
  if (compares.length) {
