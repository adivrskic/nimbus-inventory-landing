...
/* pickModel — heuristic tiered routing, no extra model call.

   Default is the strong model. We downshift to the fast/cheap model ONLY when
   a message is short AND opens like a plain informational/help question AND
   carries none of the sales / comparative / multi-step / tool-likely signals
   below. The asymmetry is deliberate: a false escalate just forgoes some
   savings, while a false downshift risks a weaker answer on a turn that might
   convert — so when in doubt we keep Sonnet.

   ESCALATE_RE matches:
     - STEMS (prefix + any continuation): pric→pricing/priced, integrat→
       integration, compar→comparison, migrat→migrate, complian→compliance,
       secur→security, plus quot, roi, budget, discount, onboard, implement,
       schedul, invoic, enterprise.
     - bounded WORDS / acronyms: vs, versus, api, webhook, sdk, sso, saml,
       hipaa, gdpr, soc 2, contract, demo, book, email, draft, compose, sales,
       human, rep, call, switch, and competitor/ERP names (fishbowl, netsuite,
       sortly, shipstation, sap, sage). Bounded so "rep" doesn't fire on
       "report", "call" not on "recall", etc.

   Note: the bare word "warehouse" and the "custom→customer" stem are
   intentionally NOT escalators — this is a warehouse product, so nearly every
   help question mentions them; matching them would route everything to Sonnet
   and defeat the tiering. The sales angle is still caught via pric / roi /
   enterprise / multi-step keywords. */
...
/* Informational/help question openers. ESCALATE_RE is always checked first, so
   "what is the pricing" / "how much does it cost" still resolve to Sonnet even
   though they open like simple questions. */
