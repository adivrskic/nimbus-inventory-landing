...
export async function checkRateLimit(visitorId) {
  ...
  if (hourly >= LIMITS.messages_per_hour) {
    return {
      ...
      retryAfter: 3600,
      message:
        "Hourly limit reached. Try again in an hour, or email sales@nautilusinventory.com.",
    };
  }
  if (daily >= LIMITS.messages_per_day) {
    return {
      ...
      retryAfter: 86400,
      message:
        "Daily message limit reached. Try again tomorrow, or email sales@nautilusinventory.com.",
    };
  }
  if (tokens >= LIMITS.tokens_per_day) {
    return {
      ...
      retryAfter: 86400,
      message:
        "Daily usage limit reached. Try again tomorrow, or email sales@nautilusinventory.com.",
    };
  }
