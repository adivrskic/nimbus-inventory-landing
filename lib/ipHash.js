...
/* Read the client IP from a request.
   ...
   Priority:
     1. x-nf-client-connection-ip — Netlify edge, client-unforgeable. The
        only entry we treat as trustworthy for rate-limiting / abuse keys.
     2. x-real-ip — some platforms set this to a single client IP. Best
        effort; only consulted when the Netlify header is absent.
     3. x-forwarded-for (left-most) — last resort for local dev. NOTE: the
        left-most entry is client-controlled and spoofable; we only reach
        here when no trusted header exists, and we keep the old left-most
        behavior so dev/test setups that synthesize XFF keep working.

   `fallback` is what to return when none are present — "unknown" for the
   form limiter (so unknown-IP traffic still gets bucketed together) or null
   for callers that prefer to skip hashing. */
