...
/* ═══════════════════════════════════════════════════════════════════════
   VOICE SCENE
   ───────────────────────────────────────────────────────────────────────
   What it shows: an audio source emitting sound waves outward, with a
   frequency-spectrum visualizer below it (the kind you'd see on a
   recording app or voice assistant).

   Spatial layout (natural orientation, before isoTransform):
     - Source: cluster at origin (0, 0, 0)
     - Sound rings: concentric circles in the XY plane, expanding outward
     - Frequency bars: 14 vertical bars below the source (extending in -Y),
       heights varying like an audio spectrum
     - Rays: thin radial lines from source outward through the rings
     - Sparks: scattered points orbiting at mid-radius, suggesting
       audio energy in motion
   ═══════════════════════════════════════════════════════════════════════ */
export function generateVoiceScene(n, rotXDeg, rotYDeg, rotZDeg) {
  const p = new Float32Array(n * 3);

  const sourceN = Math.floor(n * 0.06);
  const ringsN = Math.floor(n * 0.22);
  const barsN = Math.floor(n * 0.36);
  const raysN = Math.floor(n * 0.14);
  const sparksN = Math.floor(n * 0.11);
  /* ambient ≈ 11% */

  let idx = 0;

  /* ─── SOURCE — dense bright cluster at origin ─────────────────── */
  for (let i = 0; i < sourceN; i++) {
    const a = Math.random() * Math.PI * 2;
    const r = Math.pow(Math.random(), 0.4) * 0.32 * S;
    const z = (Math.random() - 0.5) * 0.18 * S;
    p[idx * 3] = Math.cos(a) * r + gaussRand() * 0.03 * S;
    p[idx * 3 + 1] = Math.sin(a) * r + gaussRand() * 0.03 * S;
    p[idx * 3 + 2] = z;
    idx++;
  }

  /* ─── RINGS — 5 concentric circles expanding outward ──────────── */
  const numRings = 5;
  const perRing = Math.floor(ringsN / numRings);
  const ringRMin = 0.85 * S;
  const ringRStep = 0.55 * S;
  for (let r = 0; r < numRings; r++) {
    const radius = ringRMin + r * ringRStep;
    const thickness = (0.06 + r * 0.015) * S;
    const limit =
      r === numRings - 1 ? sourceN + ringsN : sourceN + (r + 1) * perRing;
    while (idx < limit) {
      const a = Math.random() * Math.PI * 2;
      const rr = radius + gaussRand() * thickness;
      p[idx * 3] = Math.cos(a) * rr;
      p[idx * 3 + 1] = Math.sin(a) * rr;
      p[idx * 3 + 2] = gaussRand() * 0.08 * S;
      idx++;
    }
  }

  /* ─── FREQUENCY BARS — 14 vertical bars below source ─────────────
     Heights pattern echoes a real audio spectrum: peaks in the
     mid-frequencies, lower at the very low/high ends. Bars extend
     in -Y direction from a baseline below the rings. */
  const numBars = 14;
  const heights = [
    0.6, 1.2, 1.9, 2.4, 2.9, 3.3, 3.6, 3.5, 3.0, 2.4, 1.9, 1.4, 1.0, 0.6,
  ];
  const barW = 0.16 * S;
  const barD = 0.16 * S;
  const barGap = 0.08 * S;
  const totalBarW = numBars * barW + (numBars - 1) * barGap;
  const barStartX = -totalBarW / 2 + barW / 2;
  const barBaseY = -(ringRMin + (numRings - 1) * ringRStep + 0.7) * S;
  const perBar = Math.floor(barsN / numBars);
  for (let bi = 0; bi < numBars; bi++) {
    const cx = barStartX + bi * (barW + barGap);
    const h = heights[bi] * 0.45 * S;
    const limit =
      bi === numBars - 1
        ? sourceN + ringsN + barsN
        : sourceN + ringsN + (bi + 1) * perBar;
    while (idx < limit) {
      const r = Math.random();
      /* 60% of bar's particles go to the front/back/sides (edges),
         40% fill the bar volume — gives the bar a defined silhouette
         with some interior density. */
      if (r < 0.6) {
        const side = Math.floor(Math.random() * 4);
        let localX, localZ;
        if (side === 0) {
          localX = -barW / 2;
          localZ = (Math.random() - 0.5) * barD;
        } else if (side === 1) {
          localX = barW / 2;
          localZ = (Math.random() - 0.5) * barD;
        } else if (side === 2) {
          localX = (Math.random() - 0.5) * barW;
          localZ = -barD / 2;
        } else {
          localX = (Math.random() - 0.5) * barW;
          localZ = barD / 2;
        }
        const localY = -Math.random() * h;
        p[idx * 3] = cx + localX + gaussRand() * 0.02 * S;
        p[idx * 3 + 1] = barBaseY + localY + gaussRand() * 0.02 * S;
        p[idx * 3 + 2] = localZ + gaussRand() * 0.02 * S;
      } else {
        p[idx * 3] =
          cx + (Math.random() - 0.5) * barW + gaussRand() * 0.015 * S;
        p[idx * 3 + 1] = barBaseY - Math.random() * h + gaussRand() * 0.015 * S;
        p[idx * 3 + 2] = (Math.random() - 0.5) * barD + gaussRand() * 0.015 * S;
      }
      idx++;
    }
  }

  /* ─── RAYS — 12 radial lines from source outward through rings ──
     Spaced evenly around the ring system, each ray is a thin line of
     particles that emphasizes the "broadcasting outward" reading. */
  const numRays = 12;
  const rayLen = (numRings * ringRStep + ringRMin) * 1.1;
  const perRay = Math.floor(raysN / numRays);
  for (let s = 0; s < numRays; s++) {
    const angle = (s / numRays) * Math.PI * 2;
    const dx = Math.cos(angle);
    const dy = Math.sin(angle);
    const limit =
      s === numRays - 1
        ? sourceN + ringsN + barsN + raysN
        : sourceN + ringsN + barsN + (s + 1) * perRay;
    while (idx < limit) {
      /* Bias toward outer end of ray — gives a tapered "stream" look */
      const t = Math.pow(Math.random(), 0.55);
      const r = t * rayLen;
      p[idx * 3] = dx * r + gaussRand() * 0.04 * S;
      p[idx * 3 + 1] = dy * r + gaussRand() * 0.04 * S;
      p[idx * 3 + 2] = gaussRand() * 0.06 * S;
      idx++;
    }
  }

  /* ─── SPARKS — particles orbiting at varying radii, with z spread ─
     Suggests audio energy moving through 3D space. These are NOT on
     the same XY plane as the rings — they're spread across z, giving
     the scene volume. */
  for (let i = 0; i < sparksN; i++) {
    const a = Math.random() * Math.PI * 2;
    const r = (1.2 + Math.random() * 3.0) * S;
    p[idx * 3] = Math.cos(a) * r + gaussRand() * 0.1 * S;
    p[idx * 3 + 1] = Math.sin(a) * r + gaussRand() * 0.1 * S;
    p[idx * 3 + 2] = gaussRand() * 0.8 * S;
    idx++;
  }

  /* ─── AMBIENT — diffuse halo, biased toward the upper hemisphere ──
     Bars dominate the lower half, so put the ambient noise where it
     adds atmosphere rather than competing with the structured form. */
  for (let i = idx; i < n; i++) {
    const a = Math.random() * Math.PI * 2;
    const r = (Math.random() * 3.5 + 1.0) * S;
    p[i * 3] = Math.cos(a) * r + gaussRand() * 0.5;
    p[i * 3 + 1] = Math.sin(a) * r * 0.7 + gaussRand() * 0.5;
    p[i * 3 + 2] = gaussRand() * 1.0;
  }
