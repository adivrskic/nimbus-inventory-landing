...
/* ── Spatial: simple 3D cube ──
   A clean six-faced cube — 12 edges form the wireframe silhouette,
   8 vertex clusters mark the corners, 6 sparse face fills give the
   form some volume without making it look filled in.

   Allocation:
     edges     52%   12 edges of the cube (the dominant silhouette)
     vertices  12%   8 corner clusters
     faces     28%   sparse particle fill on the 6 faces
     ambient    8%   sparse halo
*/
export function generateCube(n, rotXDeg, rotYDeg, rotZDeg) {
  const p = new Float32Array(n * 3);

  /* Half-extent — cube spans ±half on every axis. 1.5*S gives a side
     of ~5.85 world units, matching the visual footprint the hex prism
     used to occupy. */
  const half = 1.5 * S;
  ...
  /* Six faces, each defined by its constant-axis and the two free axes.
     `axis` = which coordinate is locked, `sign` = +half or -half on
     that axis. Sample uniformly across the other two axes in [-half, +half]. */
  const faces = [
  ...
  const edgesN = Math.floor(n * 0.52);
  const verticesN = Math.floor(n * 0.12);
  const facesN = Math.floor(n * 0.28);
  /* ambient = ~8% remainder */
  ...
  for (let e = 0; e < 12; e++) {
    ...
    while (idx < limit) {
      const t = Math.random();
      p[idx * 3] = v1[0] + (v2[0] - v1[0]) * t + gaussRand() * 0.035 * S;
      p[idx * 3 + 1] = v1[1] + (v2[1] - v1[1]) * t + gaussRand() * 0.035 * S;
      p[idx * 3 + 2] = v1[2] + (v2[2] - v1[2]) * t + gaussRand() * 0.035 * S;
      idx++;
    }
  }

  /* ─── VERTEX CLUSTERS — 8 corner emphasis dots ──────────────── */
  const perVertex = Math.floor(verticesN / 8);
  const vxBase = edgesN;
  for (let v = 0; v < 8; v++) {
    const vert = verts[v];
    const limit = v === 7 ? vxBase + verticesN : vxBase + (v + 1) * perVertex;
    while (idx < limit) {
      p[idx * 3] = vert[0] + gaussRand() * 0.11 * S;
      p[idx * 3 + 1] = vert[1] + gaussRand() * 0.11 * S;
      p[idx * 3 + 2] = vert[2] + gaussRand() * 0.11 * S;
      idx++;
    }
  }

  /* ─── FACES — sparse fill on each of the 6 faces ─────────────
     Each face is an axis-aligned square. We lock one coordinate to
     ±half and sample the other two uniformly across [-half, +half]. */
  const faceBase = vxBase + verticesN;
  ...
  for (let f = 0; f < 6; f++) {
    ...
    while (idx < limit) {
      ...
      /* Place u and v into the two free axes; the locked axis gets the
         constant value plus a small jitter so the face surface has a
         hint of thickness rather than being mathematically flat. */
      let x, y, z;
      if (face.axis === 0) {
        x = lockedVal;
        y = u;
      ...
      p[idx * 3] =
        x + (face.axis === 0 ? gaussRand() * 0.04 * S : gaussRand() * 0.04 * S);
      p[idx * 3 + 1] =
        y + (face.axis === 1 ? gaussRand() * 0.04 * S : gaussRand() * 0.04 * S);
      p[idx * 3 + 2] =
        z + (face.axis === 2 ? gaussRand() * 0.04 * S : gaussRand() * 0.04 * S);
      idx++;
    }
  }
...
/* ── Analytics: 3D bars + trend line + ground rails ──
   ...
   ground rails below. Bars are now larger (taller + wider) so they
   command more of the canvas instead of getting lost in the field.
...
export function generateStockArrow(n, rotXDeg, rotYDeg, rotZDeg) {
  ...
  /* Heights bumped ~30% from previous version — old set peaked at 2.4,
     new set peaks at 3.1 so the tallest bar reads as a clear focal
     point rather than another speck in the field. */
  const heights = [0.8, 1.3, 1.8, 1.3, 2.1, 2.6, 2.1, 3.1];
  const numBars = heights.length;

  /* Wider bars too — was 0.34, now 0.42. With heights also up, the bar
     chart's overall mass is roughly 1.5x what it was. */
  const barW = 0.42 * S;
  const barD = 0.42 * S;
  const gap = 0.18 * S;
  ...
  /* ─── TREND LINE — smooth curve across the bar tops ───────────
     ...
     line that floats just above the bars. */
