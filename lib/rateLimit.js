...
/* getClientIp lives in lib/ipHash.js (shared with the chat path). Re-exported
   so existing `import { rateLimit, getClientIp } from "@/lib/rateLimit"`
   callers keep working unchanged. */
export { getClientIp } from "@/lib/ipHash";

/* ── In-memory degraded-mode backstop ─────────────────────────────────────
   Only used when the RPC errors. Bounded, per-process, lost on deploy. */
 ...
 * @param {number} [opts.max=5]               Requests allowed per window.
 * @param {number} [opts.windowMs=3600000]    Rolling window length (ms).
 */
